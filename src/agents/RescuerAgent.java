...
public class RescuerAgent extends Agent {
    ...
    protected void setup() {
        ...
        for (String service : sconfig.getServiceRA()) {
        ...
        for (int i = 0; i < sconfig.getServiceRA().size(); i++)
            for (int j = 0; j < sconfig.getAction_type_size(); j++) {
                String value = sconfig.getServiceRA().get(i) + ", " + sconfig.getActionName(j) + ", " + 0;
                flipvalues.add(value);
            }
        //control service completion condition
        for (int i = 0; i < sconfig.getServiceRA().size(); i++) {
            totalservicegoalvalue += sconfig.getGoal_value(sconfig.getServiceRA().get(i).trim());
            ctrlapp.put(sconfig.getServiceRA().get(i), 0);
        }
                ...
                case 1: // search reciever
                    ...
                    try {
                        ...
                        if (result.length > 0) {
                            ...
                            for (int i = 0; i < sconfig.getServiceRA().size(); i++) {
                                if (sconfig.getServiceRA().get(i).trim().equals(rqstring.get(0).trim())) {
                                    ...
                                    if (completedtask > 0) {
                                        ...
                                        mean = (float) (completedtask / sconfig.getRANUM());
                                    }
                                    ...
                                    decision.add(sconfig.getServiceRA().get(i));
