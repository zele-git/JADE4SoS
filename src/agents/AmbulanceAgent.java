...
public class AmbulanceAgent extends Agent {
    ...
    float BLEVEL = (float) sconfig.getEV_BELONGIGN_VALUE();
    ...
    protected void setup() {
        ...
        for (String service : sconfig.getServiceEV()) {
        ...
        for (int i = 0; i < sconfig.getServiceEV().size(); i++)
            for (int j = 0; j < sconfig.getAction_type_size(); j++) {
                String value = sconfig.getServiceEV().get(i) + ", " + sconfig.getActionName(j) + ", " + 0;
                flipvalues.add(value);
            }
        //control service completion condition
        for (int i = 0; i < sconfig.getServiceEV().size(); i++) {
            totalservicegoalvalue += sconfig.getGoal_value(sconfig.getServiceEV().get(i).trim());
            ctrlapp.put(sconfig.getServiceEV().get(i), 0);
        }
                ...
                case 1: // search reciever
                    ...
                    try {
                        ...
                        if (result.length > 0) {
                            ...
                            for (int i = 0; i < sconfig.getServiceEV().size(); i++) {
                                if (sconfig.getServiceEV().get(i).trim().equals(rqstring.get(0).trim())) {
                                    ...
                                    if (completedtask > 0) {
                                        ...
                                        mean = (float) (completedtask / sconfig.getEVNUM());
                                    }
                                    ...
                                    decision.add(sconfig.getServiceEV().get(i));
